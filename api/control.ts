...
async function latestRuns(limit = 10) {
  return github(`/repos/${REPO}/actions/workflows/${WORKFLOW}/runs?per_page=${Math.min(limit, 20)}`);
}

export async function GET(request: Request): Promise<Response> {
  try {
    if (!isAuthorized(request)) return json({ ok: false, error: "Unauthorized" }, 401);
    const data = await latestRuns(5);
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (body.action === "stop") {
      const data = await github(`/repos/${REPO}/actions/workflows/${WORKFLOW}/runs?status=in_progress&per_page=1`);
      const run = data.workflow_runs?.[0];
    ...
    const requestedMax = Number(body.max_applications ?? 1);
    if (!Number.isInteger(requestedMax) || requestedMax < 1 || requestedMax > 20) {
      return json({ ok: false, error: "max_applications must be an integer from 1 to 20" }, 400);
    }
    const mode = body.automation_mode ?? "semi_automated";
    if (!ALLOWED_MODES.has(mode)) return json({ ok: false, error: "Invalid automation_mode" }, 400);

    const running = await github(`/repos/${REPO}/actions/workflows/${WORKFLOW}/runs?status=in_progress&per_page=1`);
    ...
    await github(`/repos/${REPO}/actions/workflows/${WORKFLOW}/dispatches`, {
      method: "POST",
      body: JSON.stringify({ ref: "main", inputs: { max_applications: String(requestedMax), automation_mode: mode } }),
    });
